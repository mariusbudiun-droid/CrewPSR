...
function renderSettings() {
  ...
  if (notifOptions) notifOptions.style.display = n.enabled ? 'block' : 'none';

  const notifReport = document.getElementById('notifReport');
  if (notifReport) notifReport.checked = n.report !== false;

  const notifDep = document.getElementById('notifDep');
  if (notifDep) notifDep.value = n.dep || 'first';

  const notifArr = document.getElementById('notifArr');
  if (notifArr) notifArr.value = n.arr || 'last';
}
  ...
  if (type === 'roster') {
    ...
    body.innerHTML = `
      ...
      <button class="btn" onclick="
        APP.roster = parseInt(document.getElementById('chRoster').value, 10);
        save();
        renderSettings();
        renderHome();
    ...
    body.innerHTML = `
      ...
      <button class="btn" onclick="
        APP.refDate = document.getElementById('chDate').value;
        save();
        renderSettings();
        renderHome();
        renderCalendar();
    ...
    body.innerHTML = `
      <div style="font-size:13px;color:var(--text2);margin-bottom:14px;">
        Scegli come proteggere la Crew Directory.
      </div>
  ...
  if (!APP.syncLoggedIn) {
    el.innerHTML = `
      ...
      <button onclick="showSyncLogin()"
        ...
        font-family:Outfit,sans-serif;font-size:15px;font-weight:700;color:white;cursor:pointer;">
        Login / Register
      </button>
    `;
    return;
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:12px;">
      ...
      <button onclick="syncLogout();renderSyncScreen();"
        ...
        background:none;font-family:Outfit,sans-serif;font-size:12px;color:var(--text3);cursor:pointer;">
        Logout
      </button>
    </div>
    ...
    <button onclick="syncPushThenShow()"
      ...
      color:var(--blue);cursor:pointer;margin-bottom:8px;">
      Sync my roster now
    </button>

    <button onclick="showPendingRequestsBadge()"
      style="width:100%;padding:11px;border-radius:10px;border:1px solid var(--border);
      background:none;font-family:Outfit,sans-serif;font-size:13px;font-weight:600;
      color:var(--text2);cursor:pointer;margin-bottom:8px;" id="pendingReqBtn">
      Access requests
    </button>
    ...
    <button onclick="showRequestAccess()"
      style="width:100%;padding:11px;border-radius:10px;border:1px solid var(--border);
      background:none;font-family:Outfit,sans-serif;font-size:13px;font-weight:600;
      color:var(--text2);cursor:pointer;margin-bottom:8px;">
...
async function syncPushThenShow() {
  ...
  try {
    ...
    renderSyncScreen();
    if (res.ok) alert('Roster synced successfully!');
  ...
  document.getElementById('settingModalBody').innerHTML = `
    <div style="display:flex;flex-direction:column;gap:8px;margin-bottom:16px;">
      <button onclick="showSyncRegisterForm()"
        style="padding:14px;border-radius:12px;border:1.5px solid var(--blue);background:var(--blue-lt);
        font-family:Outfit,sans-serif;font-size:14px;font-weight:700;color:var(--blue);cursor:pointer;text-align:left;">
        Register first time
        <div style="font-size:11px;font-weight:400;color:var(--text2);margin-top:3px;">
          Create your account, requires approval.
        </div>
      </button>

      <button onclick="showSyncLoginForm()"
        style="padding:14px;border-radius:12px;border:1px solid var(--border);background:var(--surface);
        font-family:Outfit,sans-serif;font-size:14px;font-weight:600;color:var(--text);cursor:pointer;text-align:left;">
...
function showSyncRegisterForm() {
  ...
  document.getElementById('settingModalBody').innerHTML = `
    ...
    <button class="btn" style="margin-top:4px;" onclick="doRegister()">Register</button>
...
async function doRegister() {
  ...
  const roster = parseInt(document.getElementById('regRoster').value, 10);
  const pin = document.getElementById('regPin').value;
  const msg = document.getElementById('regMsg');

  if (!code || code.length < 3) {
    msg.textContent = 'Enter your crew code';
  ...
  if (res.ok) {
    msg.textContent = 'Registered! Waiting for approval by Marius. You will be able to login once approved.';
...
function showSyncLoginForm() {
  ...
  document.getElementById('settingModalBody').innerHTML = `
    ...
    <button class="btn" style="margin-top:4px;" onclick="doLogin()">Login</button>
    <button class="btn secondary" onclick="showSyncLogin()">Back</button>
  `;
  document.getElementById('settingModal').classList.add('open');
}
...
async function showSharingManager() {
  ...
  document.getElementById('settingModalBody').innerHTML =
    `<div style="font-size:13px;color:var(--text2);padding:8px 0;">Loading...</div>`;
  document.getElementById('settingModal').classList.add('open');
    ...
    ? shares.map(s => `
      <div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border);">
        ...
        <button onclick="syncRevokeShare('${s.crew_code}').then(showSharingManager)"
          style="padding:6px 12px;border-radius:8px;border:1px solid var(--red);background:none;
          font-family:Outfit,sans-serif;font-size:12px;color:var(--red);cursor:pointer;">
          Revoke
        </button>
      </div>
    `).join('')
  ...
  document.getElementById('settingModalBody').innerHTML = `
    ...
    <button class="btn" onclick="doShare()" style="margin-bottom:8px;">Share</button>
...
async function doShare() {
  ...
  if (res.ok) {
    msg.textContent = `Now sharing with ${res.name}`;
    msg.style.color = 'var(--green)';
    msg.style.display = 'block';
...
async function showRequestAccess() {
  ...
  document.getElementById('settingModalBody').innerHTML = `
    ...
    <button class="btn" onclick="doRequestAccess()" style="margin-top:4px;">Send request</button>
    <button class="btn secondary" onclick="closeModal('settingModal')">Cancel</button>
  `;
  document.getElementById('settingModal').classList.add('open');
}
...
async function showPendingRequests() {
  ...
  document.getElementById('settingModalBody').innerHTML =
    `<div style="color:var(--text2);font-size:13px;padding:8px 0;">Loading...</div>`;
  document.getElementById('settingModal').classList.add('open');
  ...
  const rows = reqs.map(r => `
    <div style="padding:12px 0;border-bottom:1px solid var(--border);">
      ...
      <div style="display:flex;gap:8px;">
        ...
        <button onclick="approveReq('${r.request_id}','${r.requester_id}',true)"
          style="flex:1;padding:9px;border-radius:9px;border:none;background:var(--green);
          font-family:Outfit,sans-serif;font-size:12px;font-weight:700;color:white;cursor:pointer;">
          Share both ways
        </button>
        <button onclick="declineReq('${r.request_id}')"
          style="padding:9px 12px;border-radius:9px;border:1px solid var(--border);background:none;
          font-family:Outfit,sans-serif;font-size:12px;color:var(--text3);cursor:pointer;">
          ✕
        </button>
      </div>
    </div>
...
async function approveReq(reqId, requesterId, mutual) {
  await syncApproveRequest(reqId, requesterId, mutual);
  showPendingRequests();
}
...
// Expose supa for admin calls from settings
window.supa = typeof _supa !== 'undefined' ? _supa : null;
...
async function openSharedView() {
  ...
  if (!profiles.length) {
    alert('No one has shared their roster with you yet. Send a request first, or ask a colleague to share.');
    return;
  }
  showSharedViewPicker(profiles);
}
window.openSharedView = openSharedView;

function showSharedViewPicker(profiles) {
  ...
  const rows = profiles.map(p => `
    <button onclick="closeModal('settingModal');openSharedSlides('${p.crew_code}','${p.display_name}',${p.roster_num},'${p.id}')"
      style="width:100%;display:flex;align-items:center;gap:12px;padding:14px;border-radius:12px;border:1px solid var(--border);
      background:var(--surface);font-family:Outfit,sans-serif;cursor:pointer;text-align:left;margin-bottom:8px;">
      <div style="background:var(--blue);color:white;width:36px;height:36px;border-radius:10px;
...
async function openSharedSlides(crewCode, displayName, rosterNum, profileId) {
  ...
  screen.innerHTML = `
    <div style="display:flex;align-items:center;gap:10px;padding:14px 16px;
      padding-top:max(14px,env(safe-area-inset-top));background:var(--surface);
      ...
      <button onclick="document.getElementById('sharedRosterScreen').remove()"
        ...
        background:var(--bg);font-family:Outfit,sans-serif;font-size:14px;font-weight:600;
    ...
    <div id="sharedLoading" style="flex:1;display:flex;align-items:center;justify-content:center;
      ...
      <div style="width:32px;height:32px;border:3px solid var(--border);
        border-top-color:var(--blue);border-radius:50%;animation:spin 0.8s linear infinite;"></div>
  ...
  const sliderHtml = `
    ...
    <div style="flex:1;overflow:hidden;position:relative;" id="sharedSliderWrap">
      <div id="sharedSlides" style="display:flex;height:100%;transition:transform 0.3s ease;will-change:transform;"></div>
    </div>
  `;

  screen.insertAdjacentHTML('beforeend', sliderHtml);
  buildSharedSlides(assignments, rosterNum, crewCode);
}
window.openSharedSlides = openSharedSlides;

function buildSharedSlides(assignments, rosterNum, crewCode) {
  ...
  function getScheduleFlightsForAssignment(ds, assignment) {
    if (!['A1E', 'A1L', 'A2E', 'A2L'].includes(assignment)) return [];
    const dow = new Date(`${ds}T12:00:00`).getDay();
    const sched = SCHEDULE?.days?.[dow];
    if (!sched) return [];

    const useA2 = assignment.startsWith('A2');
    const useLate = assignment.endsWith('L');
  ...
  function getReportForAssignment(ds, assignment) {
    if (!['A1E', 'A1L', 'A2E', 'A2L'].includes(assignment)) return null;
    const dow = new Date(`${ds}T12:00:00`).getDay();
    const sched = SCHEDULE?.days?.[dow];
    ...
    return useLate ? plane?.reportLate : plane?.reportEarly;
  }
  ...
  dates.forEach((ds, idx) => {
    ...
    if (entry && entry.assignment && entry.assignment !== 'OFF') {
      ...
      if (a === 'HSBY') {
        ...
        if (entry.details?.start || entry.details?.end) {
          dutyDetails = `<div style="font-size:12px;opacity:0.8;margin-top:2px;">
            ${entry.details?.start || ''}${entry.details?.end ? ` - ${entry.details.end}` : ''}
          </div>`;
        }
      } else if (a === 'AD') {
        ...
        dutyLabel = 'Annual Leave';
        dutyColor = 'var(--off)';
      } else if (a === 'VTO') {
        dutyLabel = 'Voluntary Time Off';
        dutyColor = 'var(--off)';
      } else if (a === 'SICK') {
        dutyLabel = 'Sick Leave';
        dutyColor = 'var(--red)';
      } else if (a === 'UL') {
        dutyLabel = 'Unpaid Leave';
        dutyColor = 'var(--off)';
      } else if (a === 'PL') {
        dutyLabel = 'Parental Leave';
        dutyColor = 'var(--off)';
      } else if (a === 'CUSTOM') {
        dutyLabel = 'Custom Flights';
        dutyColor = 'var(--blue)';
        displayFlights = (entry.flights || []).filter(f => f && f.from && f.to);
      } else if (a === 'A1E') {
        dutyLabel = 'Aereo 1 Early';
        dutyColor = 'var(--early)';
        displayFlights = getScheduleFlightsForAssignment(ds, a);
        reportTime = getReportForAssignment(ds, a);
      } else if (a === 'A1L') {
        dutyLabel = 'Aereo 1 Late';
        dutyColor = 'var(--late)';
        displayFlights = getScheduleFlightsForAssignment(ds, a);
        reportTime = getReportForAssignment(ds, a);
      } else if (a === 'A2E') {
        dutyLabel = 'Aereo 2 Early';
        dutyColor = 'var(--early)';
        displayFlights = getScheduleFlightsForAssignment(ds, a);
        reportTime = getReportForAssignment(ds, a);
      } else if (a === 'A2L') {
        dutyLabel = 'Aereo 2 Late';
        dutyColor = 'var(--late)';
        displayFlights = getScheduleFlightsForAssignment(ds, a);
        reportTime = getReportForAssignment(ds, a);
      } else {
    ...
    const flightHtml = displayFlights.length
    ...
    const reportHtml = reportTime
      ? `<div style="font-size:11px;color:var(--text3);margin-top:4px;font-family:JetBrains Mono,monospace;">
          Report ${reportTime}
        </div>`
      : '';
    ...
    slide.innerHTML = `
      <div style="background:var(--surface);margin:12px 16px 0;border-radius:16px;
        ...
        <div style="padding:16px;border-bottom:1px solid var(--border);display:flex;
          ...
          <div>
            <div style="font-size:11px;font-weight:700;color:var(--text3);text-transform:uppercase;letter-spacing:1px;">
              ${dow}
            </div>
            <div style="font-size:26px;font-weight:800;color:${heroClass};line-height:1.1;">
              ${d.getDate()} ${MONTHSS[d.getMonth()]}
            </div>
    ...
    dot.style.cssText = `
      ...
      height:${isToday ? '8px' : '6px'};
      border-radius:50%;
      background:${isToday ? 'var(--blue)' : 'var(--border2)'};
      cursor:pointer;
      transition:all 0.2s;
    `;
